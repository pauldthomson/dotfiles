...
- Uses the Catppuccin theme with higher-contrast separators, plus a `Ctrl+Space` prefix to preserve the established terminal appearance and muscle memory during the tmux migration. `prefix + j` opens the fuzzy workspace picker, `prefix + d` detaches, `prefix + r` reloads the configuration (`prefix + Shift+r` enters resize mode), and `Ctrl + h/j/k/l` focuses the pane left/down/up/right. Pane splits also match tmux: `prefix + -` splits horizontally and `prefix + |` splits vertically.
...
- `prefix + j`: switch sessions (fzf popup with wraparound navigation).
...
- `Ctrl+Shift+Up`/`Ctrl+Shift+Down` are forwarded to applications while the alternate screen is active, allowing Pi to navigate its fullscreen transcript. Outside fullscreen applications, tmux retains its copy-mode prompt navigation.
...
- `fff.nvim` follows the upstream setup: the plugin loads at startup, lazy-initializes its index internally, and provides file-finding and grep keymaps (`ff`, `fg`, `fz`, and `fw`).
